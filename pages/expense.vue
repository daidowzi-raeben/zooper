...
<template>
  <div class="pb-32 space-y-10 min-h-screen">
    <!-- 💸 출금 헤더 (Expense Header) -->
    <div class="relative bg-gradient-to-br from-rose-500 to-orange-600 rounded-[40px] p-8 text-white shadow-2xl shadow-rose-100 overflow-hidden">
      ...
      <div class="relative z-10 flex flex-col items-center text-center space-y-6">
        <div class="w-20 h-20 bg-white/20 rounded-3xl flex items-center justify-center backdrop-blur-md border border-white/20 shadow-xl">
          <Icon name="solar:wallet-money-bold-duotone" class="w-12 h-12 text-white" />
        </div>
    ...
    <div class="bg-white p-8 rounded-[40px] border border-gray-100 shadow-xl space-y-8">
      ...
      <div class="space-y-6">
        <div class="relative group">
          ...
          />
          <span class="absolute left-6 top-1/2 -translate-y-1/2 i-heroicons-shopping-cart-solid w-7 h-7 text-gray-300 group-focus-within:text-rose-500 transition-colors" />
        </div>
    ...
    <!-- 📱 QR 스캐너 모달 (오버레이) -->
    <div v-if="isScanning" class="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md p-10">
      <div class="absolute top-10 text-white text-center space-y-2">
        <p class="text-[10px] font-black tracking-widest uppercase opacity-60">Authorize Transaction</p>
        <h3 class="text-3xl font-black text-rose-400">{{ Number(amountInput).toLocaleString() }} {{ dispot?.currency_name || '돌멩이' }}</h3>
        <p class="text-sm font-medium text-white/50 text-rose-200">은행원의 QR코드를 스캔해주세요</p>
      </div>

      <div class="relative w-full max-w-sm aspect-square rounded-[40px] overflow-hidden border-4 border-rose-500/30 shadow-2xl">
        ...
        <div class="absolute top-1/2 left-0 right-0 h-0.5 bg-rose-500/50 shadow-[0_0_20px_rgba(244,63,94,1)] animate-scan"></div>
      </div>

      <button @click="isScanning = false" class="mt-12 text-white/50 hover:text-white font-black text-sm p-4 tracking-widest uppercase">Go Back</button>
    </div>
    ...
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-[40px] border border-gray-100 shadow-sm overflow-hidden divide-y divide-gray-50/50 text-gray-800">
        <div v-if="points.length > 0">
          <div v-for="item in points" :key="item.idx" class="flex items-center justify-between p-6 hover:bg-rose-50/30 transition-all group">
            <div class="flex items-center gap-4">
               <div :class="['w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all group-hover:scale-110', 
                 ...
                 <Icon :name="item.point_type === 'save' ? 'solar:arrow-down-bold' : 'solar:arrow-up-bold'" class="w-6 h-6" />
               </div>
               <div>
            ...
            <div class="text-right">
               <p :class="['text-base font-black tabular-nums', item.point_type === 'save' ? 'text-blue-600' : 'text-rose-600']">
